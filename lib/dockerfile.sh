...
EOF
    ...
    java)
      cat <<EOF
# dockauto-template-version: ${template_version}
# Generated by dockauto for Java (Spring Boot + Maven Wrapper)
FROM eclipse-temurin:${language_version:-21}-jdk
...
# (Optional) tải dependency offline để cache layer
RUN chmod +x mvnw && ./mvnw -q dependency:go-offline || true

# Copy source
COPY src src

# Build jar (skip tests, tests sẽ chạy ở Step 8)
